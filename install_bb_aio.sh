#!/bin/sh
# install_bb_aio_v8_trimmed_targets_full_repo.sh
# Bash Bunny updater:
# - downloads FULL Jessie armhf repo cache from manifest so APT has dependencies
# - installs ONLY the trimmed target package list using local APT dependency resolver
# - installs Bunny custom tool debs
# - avoids brute-force dpkg ordering for Jessie packages

set -u

echo "[MARKER] running install_bb_aio_v8_trimmed_targets_full_repo.sh"
...
disable_bad_default_release() {
  # Fixes: E: The value 'jessie' is invalid for APT::Default-Release
  # without permanently deleting user config. We move only files containing Default-Release.
...
repair_broken_state() {
  log "Repairing existing broken dpkg/APT state if needed"

  # procps/udev were reported broken in the previous run. Reinstall from local cache
  # before asking apt to resolve the trimmed target set.
  for pkg in procps udev; do
    deb="$(ls "$DEBS_DIR"/${pkg}_*.deb 2>/dev/null | head -1)"
...
verify() {
  ...
  for deb in "$DEBS_DIR"/*.deb "$TOOLS_DIR"/*.deb; do
    [ -e "$deb" ] || continue
    if ! validate_deb "$deb"; then
      # macchanger tar.gz is not in this glob because it is not .deb.
      warn "BAD DEB: $deb"
      bad=$((bad + 1))
    fi
  done
  ...
  for cmd in gcc g++ make curl wget nmap tcpdump tmux htop socat strace lsof radare2 macchanger applytheme reloadtheme; do
